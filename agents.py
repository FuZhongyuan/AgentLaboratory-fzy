    ...
    def command_descriptions(self, phase):
        ...
        if phase == "data preparation":
            return (
                ...
                "At the beginning of your data preparation code, you should include code to survey the Python environment to identify available hardware acceleration features (like GPU, CPU capabilities). This will help optimize subsequent experiments. Specifically:\n"
                "1. Check for available GPU devices and their properties\n"
                "2. Detect CPU capabilities and available memory\n"
                "3. Check for available deep learning frameworks and their versions. For example, PyTorch.\n"
                "4. Avoid using parallel processing, multiprocessing, or multi-worker data loading techniques that might cause instability\n"
                "5. Use standard, widely compatible methods for hardware acceleration\n"
                "When generating visualizations and plots:\n"
                "1. Use matplotlib, seaborn, or other visualization libraries to create informative and visually appealing plots\n"
                "2. Save all generated images using plt.savefig() with descriptive filenames (e.g., 'data_distribution.png', 'model_accuracy.png')\n"
                "3. Include plt.show() after plt.savefig() to display the plots during execution\n"
                "4. Add appropriate titles, labels, and legends to all visualizations\n"
                "5. Consider creating multiple visualizations to illustrate different aspects of the data or results\n"
                "6. For interactive visualizations, ensure they are properly saved as static images as well\n"
                "IMPORTANT: Your code must be designed to run within 2400 seconds (40 minutes). To ensure this:\n"
                "1. Choose small datasets or small subsets of datasets (limit samples to a few thousand)\n"
                "2. Use minimal training epochs/iterations for model training (5-10 epochs is usually sufficient for demonstration)\n"
                "3. Reduce model complexity when possible (fewer layers, smaller hidden dimensions)\n"
                "4. Add timing code to monitor execution time and print progress updates\n"
                "5. Focus on demonstrating the concept rather than achieving state-of-the-art performance\n"
                "You can only use a SINGLE command per inference turn. Do not use more than one command per inference. If you use multiple commands, then only one of them will be executed, NOT BOTH.\n"
    ...
    def command_descriptions(self, phase):
        ...
        if phase == "data preparation":
            return (
                ...
                "The submitted code should include environment survey code at the beginning to identify available hardware acceleration features (like GPU, CPU capabilities). This will help optimize subsequent experiments. Specifically, the code should:\n"
                "1. Check for available GPU devices and their properties\n"
                "2. Detect CPU capabilities and available memory\n"
                "3. Check for available deep learning frameworks and their versions. For example, PyTorch.\n"
                "4. Avoid using parallel processing, multiprocessing, or multi-worker data loading techniques that might cause instability\n"
                ...
                "5. Consider creating multiple visualizations to illustrate different aspects of the data or results\n"
                "6. For interactive visualizations, ensure they are properly saved as static images as well\n"
                "IMPORTANT: The submitted code must be designed to run within 2400 seconds (40 minutes). To ensure this:\n"
                "1. Choose small datasets or small subsets of datasets (limit samples to a few thousand)\n"
                "2. Use minimal training epochs/iterations for model training (5-10 epochs is usually sufficient for demonstration)\n"
                "3. Reduce model complexity when possible (fewer layers, smaller hidden dimensions)\n"
                "4. Add timing code to monitor execution time and print progress updates\n"
